 ...
 if (isset($_POST['submit'])) {
      ...
      if (empty($_POST['book_name'])) {
        ...
        if (!preg_match($name_pattern,$bookname)) {
           $bookname_error ="please enter valid product name";
        }
      }
        //validate Author
        if (empty($_POST['author'])) {
           $author_error = "please enter distributor name";
        }else {
          ...
          if(empty($bookname_error) && empty($author_error) && empty($price_error) && empty($category_error) && empty($image_err))
          {
            ...
            if (mysqli_query($link,$sql)) {
               $status = '<div class="alert alert-success">successfully added product</div>';
            }else {
             $status = '<div class="alert alert-success">Error adding product</div>';
            }
          }
     ...
     <div class="col-lg-12">
       <div class="row">
         <div class="col-sm-3"> </div>
            <div class="col-sm-5">
              <h4 class="text-warning">Provide below details to add product</h4><br>
